    ...
    with open(input_file, "r") as f:
        ...
        for k, v in j.items():
            match v["type"]:
                ...
                case "CC":
                    squares.append(Square.CardSquare(k, v["name"], community_chest, game))
                case "CH":
...
class Game:
    def __init__(self, squares: List[Square.Square], chance_cards: List, community_chest_cards: List) -> None:
        self.players = []
    ...
    def show_menu(self, player, rolled: bool = False):
        ...
        while action not in choices:
            action = input(f"\033[38;2;{player._hex_color[0]};{player._hex_color[1]};{player._hex_color[2]}mChoose an action: {', '.join(choices)}\033[0m\n")
    ...
    def roll_and_move(self, player: Player.Player):
        if not player._jailed:
            ...
            self.player_positions[player] = (self.player_positions[player] + total_roll) % len(self.squares)
            print(f"New position: {self.player_positions[player]}")
            if type(s := self.squares[self.player_positions[player]]) == Square.Property:
                colors = self.colors[s.group]
                print(f"\033[48;2;{colors[0]};{colors[1]};{colors[2]}m{self.squares[self.player_positions[player]].name}\033[0m\n")
            else:
